...
std::vector<Point> Build_Geometry::compute_profile(unsigned int & cut_pos) const{
    ...
    //we now compute the y coordinates of the upper and lower profile, we take into account how many points we want to cut
    ...
    Points.reserve(2*this->my_data.NACA_points -1-this->cutted_points);

    for(size_t i = 0; i<this->my_data.NACA_points-this->cutted_points; ++i){

        Point temp( x_coord[i] ,  up_coord[i] ,0.0, weights[i]);    
        Points.push_back(temp);

    }
    cut_pos = Points.size() -1;
    for(size_t i = this->my_data.NACA_points - this->cutted_points -1; i>0; --i){
...
//-----------------------------------------------------------------------------------------------------------------------------------------------
void Build_Geometry::compute_trapezoid( Point & p_in ,double h,double b, double &x_coord, double &y_coord ) const{
...
void Build_Geometry::write_profile(std::ofstream & ofs) const{
    ...
    //we create a variable that'll store the position of the cut
    unsigned int cut_pos = 0;

    //first we retrive the points that we need
    std::vector<Point> airfoil_points = this->compute_profile(cut_pos); 
    ...
    // now we compute the points for the trapezoid shape and we print them
    double x_trap = 0;
    double y_trap = 0;
    compute_trapezoid( airfoil_points[cut_pos], 0.001, 0.0005, x_trap, y_trap );
    Point P_trap_1(x_trap , y_trap ,0.0,this->my_data.mesh_ref_2);
    Point P_trap_2(x_trap , -y_trap ,0.0,this->my_data.mesh_ref_2);

    ofs << "Point("<<P_trap_1.get_tag()<<") = {"<<P_trap_1.get_x()<< ","<<P_trap_1.get_y()<< ","<<P_trap_1.get_z()<< ", mesh_ref_2};"<<std::endl;
    ofs << "Point("<<P_trap_2.get_tag()<<") = {"<<P_trap_2.get_x()<< ","<<P_trap_2.get_y()<< ","<<P_trap_2.get_z()<< ", mesh_ref_2};"<<std::endl;
    ...
    ofs << "Spline(2) = {"<<airfoil_points[h-1].get_tag()<<":"<<airfoil_points[cut_pos].get_tag()<<"};"<<std::endl;

    ofs << "Line(3) = {"  <<airfoil_points[cut_pos].get_tag()<<","<<P_trap_1.get_tag()<<"};"<<std::endl;
    ofs << "Line(4) = {"  <<P_trap_1.get_tag()<<","<<P_trap_2.get_tag()<<"};"<<std::endl;
    ofs << "Line(5) = {"  <<P_trap_2.get_tag()<<","<<airfoil_points[cut_pos+1].get_tag()<<"};"<<std::endl;

    ofs << "Spline(6) = {"<<airfoil_points[cut_pos+1].get_tag()<<":"<<airfoil_points[3*h-1].get_tag()<<"};"<<std::endl;
    ofs << "Spline(7) = {"<<airfoil_points[3*h-1].get_tag()<<":"<<airfoil_points[airfoil_points.size()-1].get_tag()<<", "<<airfoil_points[0].get_tag()<<"};"<<std::endl;
...
void Build_Geometry::write_emitter(std::ofstream & ofs) const{
    ...
    ofs << "Circle(8) = {" << emitter_points[1].get_tag() <<"   , "<<emitter_points[0].get_tag()<<"   , "<<emitter_points[2].get_tag()<<"};"<<std::endl;
    ofs << "Circle(9) = {" << emitter_points[2].get_tag() <<"   , "<<emitter_points[0].get_tag()<<"   , "<<emitter_points[3].get_tag()<<"};"<<std::endl;
    ofs << "Circle(10) = {" << emitter_points[3].get_tag() <<"   , "<<emitter_points[0].get_tag()<<"   , "<<emitter_points[4].get_tag()<<"};"<<std::endl;
    ofs << "Circle(11) = {" << emitter_points[4].get_tag() <<"   , "<<emitter_points[0].get_tag()<<"   , "<<emitter_points[1].get_tag()<<"};"<<std::endl;
...
void Build_Geometry::write_domain(std::ofstream & ofs) const{
    ...
    ofs << "Line(12) = {"<<domain_points[0].get_tag()<<", "<<domain_points[1].get_tag()<<"};"<<std::endl;
    ofs << "Line(13) = {"<<domain_points[1].get_tag()<<", "<<domain_points[2].get_tag()<<"};"<<std::endl;
    ofs << "Line(14) = {"<<domain_points[2].get_tag()<<", "<<domain_points[3].get_tag()<<"};"<<std::endl;
    ofs << "Line(15) = {"<<domain_points[3].get_tag()<<", "<<domain_points[0].get_tag()<<"};"<<std::endl;
...
void Build_Geometry::write_loops(std::ofstream & ofs) const{
    ...
    ofs <<"Curve Loop(1) = {12, 13, 14, 15};"<<std::endl; //curve loop of the domain counter-clokwise

    ofs <<"Curve Loop(2) = {8,9,10,11};"<<std::endl; //curve loop of the circular emitter counter-clockwise

    ofs <<"Curve Loop(3) = {-1, -7, -6, -5, -4, -3 ,-2};"<<std::endl; //curve loop of the airfoil counter-clockwise
...
void Build_Geometry::write_physical_groups(std::ofstream & ofs) const{
    ...
    ofs << "Physical Curve(1)={12}; //Inlet"<<std::endl;                   //Physical curve with tag 1 is the inlet
    ofs << "Physical Curve(2)={14}; //Outlet"<<std::endl;                  //Physical curve with tag 2 is the outlet
    ofs << "Physical Curve(3)={8,9,10,11}; //Emitter"<<std::endl;         //Physical curve with tag 3 is the emitter
    ofs << "Physical Curve(4)={-1, -7, -6, -5, -4, -3, -2}; //Airfoil"<<std::endl; //Physical curve with tag 4 is the airfoil
...
void Build_Geometry::write_BL_airfoil(std::ofstream & ofs) const{
    ...
    ofs << "Field[1].CurvesList = {1,2,3,4,5,6,7};"<<std::endl;                                  //Tags of curves in the geometric model for which a boundary layer is needed
...
void Build_Geometry::write_BL_emitter(std::ofstream & ofs) const{
    ...
    ofs << "Field[2].CurvesList = {8,9,10,11};"<<std::endl;                                  //Tags of curves in the geometric model for which a boundary layer is needed
