...
void Build_Geometry::write_profile(std::ofstream & ofs) const{
    ...
    // we create a last point that will be used as the center of the circunference that substitute the trailing edge (questi due comadi
    //vengono dal tentativo di far terminare la trailing edge with a cirfunference )
    //Point circ_center( airfoil_points[cut_pos].get_x() ,  0.0 ,0.0, this->my_data.mesh_ref_1);

    // then we write it
    //ofs << "Point("<<circ_center.get_tag() <<") = {"<<circ_center.get_x()<<","<<circ_center.get_y()<<","<<circ_center.get_z()<<","<<circ_center.get_local_mesh_ref()<<"};"<<std::endl;
    ...
    ofs << "Line(3) = {"  <<airfoil_points[cut_pos].get_tag()<<","<<airfoil_points[cut_pos +1].get_tag()<<"};"<<std::endl;
    ofs << "Spline(4) = {"<<airfoil_points[cut_pos+1].get_tag()<<":"<<airfoil_points[3*h-1].get_tag()<<"};"<<std::endl;
    ofs << "Spline(5) = {"<<airfoil_points[3*h-1].get_tag()<<":"<<airfoil_points[airfoil_points.size()-1].get_tag()<<", "<<airfoil_points[0].get_tag()<<"};"<<std::endl;
...
void Build_Geometry::write_emitter(std::ofstream & ofs) const{
    ...
    ofs << "Circle(6) = {" << emitter_points[1].get_tag() <<"   , "<<emitter_points[0].get_tag()<<"   , "<<emitter_points[2].get_tag()<<"};"<<std::endl;
    ofs << "Circle(7) = {" << emitter_points[2].get_tag() <<"   , "<<emitter_points[0].get_tag()<<"   , "<<emitter_points[3].get_tag()<<"};"<<std::endl;
    ofs << "Circle(8) = {" << emitter_points[3].get_tag() <<"   , "<<emitter_points[0].get_tag()<<"   , "<<emitter_points[4].get_tag()<<"};"<<std::endl;
    ofs << "Circle(9) = {" << emitter_points[4].get_tag() <<"   , "<<emitter_points[0].get_tag()<<"   , "<<emitter_points[1].get_tag()<<"};"<<std::endl;
...
void Build_Geometry::write_domain(std::ofstream & ofs) const{
    ...
    ofs << "Line(10) = {"<<domain_points[0].get_tag()<<", "<<domain_points[1].get_tag()<<"};"<<std::endl;
    ofs << "Line(11) = {"<<domain_points[1].get_tag()<<", "<<domain_points[2].get_tag()<<"};"<<std::endl;
    ofs << "Line(12) = {"<<domain_points[2].get_tag()<<", "<<domain_points[3].get_tag()<<"};"<<std::endl;
    ofs << "Line(13) = {"<<domain_points[3].get_tag()<<", "<<domain_points[0].get_tag()<<"};"<<std::endl;
...
void Build_Geometry::write_loops(std::ofstream & ofs) const{
    ...
    ofs <<"Curve Loop(1) = {10, 11, 12, 13};"<<std::endl; //curve loop of the domain counter-clokwise

    ofs <<"Curve Loop(2) = {6, 7, 8, 9};"<<std::endl; //curve loop of the circular emitter counter-clockwise

    ofs <<"Curve Loop(3) = {-1, -5, -4, -3, -2};"<<std::endl; //curve loop of the airfoil counter-clockwise
...
void Build_Geometry::write_BL_airfoil(std::ofstream & ofs) const{
    ...
    ofs << "Field[1].CurvesList = {1,2,3,4,5};"<<std::endl;                                  //Tags of curves in the geometric model for which a boundary layer is needed
...
void Build_Geometry::write_BL_emitter(std::ofstream & ofs) const{
    ...
    ofs << "Field[2].CurvesList = {6,7,8,9};"<<std::endl;                                  //Tags of curves in the geometric model for which a boundary layer is needed
